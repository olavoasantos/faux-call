...
const hasOneDeleteResponse = (Model, Relationship) => (req, res) => {
  ...
  if (row[Model.hasMany[Relationship.name]] !== parseInt(req.params.id)) return res.status(500).send(JSON.stringify(`${Relationship.name} does not belong to specified ${Model.name}`));
